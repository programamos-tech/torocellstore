...
export function setZonatSessionCookie(jsonPayload: string): void {
  ...
  document.cookie = `torocell_user=${encodeURIComponent(jsonPayload)}; path=/; max-age=${SESSION_COOKIE_MAX_AGE_SEC}; SameSite=Lax`
}

export function clearZonatSessionCookie(): void {
  if (typeof document === 'undefined') return
  document.cookie = 'torocell_user=; path=/; expires=Thu, 01 Jan 1970 00:00:00 GMT'
}
...
export function persistZonatUser(user: User): void {
  ...
  localStorage.setItem('torocell_user', json)
