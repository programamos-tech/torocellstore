...
export function AuthProvider({ children }: { children: ReactNode }) {
  ...
  const clearClientSession = useCallback(() => {
    localStorage.removeItem('torocell_user')
    ...
    const checkAuth = async () => {
      try {
        if (typeof window !== 'undefined') {
          const savedUser = localStorage.getItem('torocell_user')
          if (savedUser) {
  ...
  useEffect(() => {
    const onStorage = (e: StorageEvent) => {
      if (e.key !== 'torocell_user' || e.storageArea !== localStorage) return
      if (e.newValue === null) {
        setUser(null)
        clearZonatSessionCookie()
        return
  ...
  useEffect(() => {
    ...
    const refreshCookieFromStorage = () => {
      const raw = localStorage.getItem('torocell_user')
